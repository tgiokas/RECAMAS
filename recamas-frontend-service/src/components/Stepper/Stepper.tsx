...
import { caseStepItems } from './data.ts';
    ...
    <>
      {stepperConfigs.map(({ id, labelId, title, fields }: StepperConfig) => (
        <div key={id} className="stepper-wrapper">
          <div id={labelId} className="stepper-label">
            {title}
          </div>
          <Stepper
            id={id}
            ...
            rtlEnabled={rtlMode}
          >
            {caseStepItems.map((step: StepperTypes.Item, index: number) => {
              const itemProps = fields.reduce(
                (
                  acc: Record<string, unknown>,
                  field: keyof StepperTypes.Item,
